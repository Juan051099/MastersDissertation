program optvorma
    ...
    use optvormod, only: pdata_type,proj,evalfg,approxg,drawsol
    ...
    real(kind=8), parameter :: maxdisbs = 1.0d-01       !max dis between sites.
    ...
    if ( f .lt. fbest ) then
        ...
        write(*,*) 'Solution was imprioved. New best f= ', fbest

        write(filename,"(A13,I0,A3)") 'fig-test-ini-',nsites,'.mp'
        call drawsol(n,x0,filename,c_loc(pdata))

        write(filename,"(A13,I0,A3)") 'fig-test-end-',nsites,'.mp'
        call drawsol(n,x,filename,c_loc(pdata))

        write(filename,"(A8,I0,A4)") 'tabline-',nsites,'.txt'
